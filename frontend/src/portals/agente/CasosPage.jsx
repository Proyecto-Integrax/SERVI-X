...
export function CasosPage() {
  ...
  async function cargar() {
    setLoading(true)
    const data = await api.listarCasos({ texto, estado, tipoSolicitud })
    setCasos(data)
    setLoading(false)
  }
    ...
    <AppShell>
      ...
      <div className="panel">
        ...
        {loading ? <p>Cargando...</p> : (
          <table>
            <thead>
              <tr><th>ID de PQR</th><th>Tipo</th><th>Fecha</th><th>Estado</th><th>Acciones</th></tr>
            </thead>
            <tbody>
              {casos.map((c) => (
                <tr key={c.casoId}>
              ...
              {casos.length === 0 && <tr><td colSpan={5}>No hay casos que coincidan con el filtro.</td></tr>}
            </tbody>
          </table>
        )}
      </div>
